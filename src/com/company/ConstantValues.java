...
class ConstantValues {
    static int iteratorColumn1 = 0, iteratorColumn2 = 0;
    ...
    static String[] QUERY = {"select season, count(id) as match_played from matches group by season order by season",
            ...
            "select delivery.bowling_team, sum(cast(extra_runs as int) ) as extra_runs from delivery where match_id in" +
                    " (select matches.id from matches where season = " + year + ") group by bowling_team",
            "select matches.winner, sum(cast(matches.win_by_runs as int))as match_win from matches where season = " + year + " group by winner order by winner;",
            "select  delivery.bowler, sum(cast(total_runs as int))/cast((count(delivery.bowler)/"+over+")as float) as economical_rate from delivery inner join matches m on delivery.match_id = m.id and m.season= " + PRE_YEAR + " group by bowler order by economical_rate;"};
}