...
class ConstantValues {
    ...
    static String[] QUERY = {"select season, count(id) as match_played from matches group by season order by season;",
            ...
            "select  delivery.bowler, sum(cast(total_runs as int))/cast((count(delivery.bowler)/"+over+")as float) as economical_rate from delivery inner join matches m on delivery.match_id = m.id and m.season=? group by bowler order by economical_rate limit "+limit+";"};
